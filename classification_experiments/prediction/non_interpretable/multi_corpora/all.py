...
for feat_name in feats_names:
    ...
    for n_fold in range(1, 11):
        ...
        Xtrain = np.concatenate([normalized_train_en, normalized_train_zh, normalized_train_china,
                                 normalized_train_lu, normalized_train_del, normalized_train_adr,
                                 ], axis=0)

        y_train = np.concatenate([y_train_en, y_train_zh, y_train_china, y_train_lu, y_train_del, y_train_adr], axis=0)

        Xval = np.concatenate([normalized_val_en, normalized_val_zh
                               ], axis=0)
        y_val = np.concatenate([y_val_en, y_val_zh], axis=0)
        Xtest = np.concatenate([normalized_test_en, normalized_test_zh
                                ], axis=0)
        y_test = np.concatenate([y_test_en, y_test_zh
                                 ], axis=0)

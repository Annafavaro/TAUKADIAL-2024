...
all_nls_elems = [os.path.join(chinese_transcripts, elem) for elem in os.listdir(chinese_transcripts)]

nls_folds = []

for elem in all_nls_elems:
    name_nls = (os.path.basename(elem).split('.txt')[0])
    if name_nls != 'HC_F_019204_004':
        lab = name_nls.split("_")[0]
        lab = [0 if lab == 'MCI' else 1]
        nls_folds.append([name_nls, open((elem)).read(), lab[0]])
